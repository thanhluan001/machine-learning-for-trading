...
_p63 = HERE / "63_force_refresh_backtest.py"  # top-level neighbor
if not _p63.exists():
...
def main() -> None:
    print("=" * 100)
    print("RC-12b PHASE 3 — pt-in-time survivorship-clean SP600, frozen V6 transfer")
    print("=" * 100)
    d = pd.read_hdf(DB_SP600, "/features/train_matrix_sp600_pt")
    print(f"events: {len(d):,}")
    d = score_v6(d)
    print(f"scored: min-gate >= {THRESH}: {(d.score >= THRESH).sum()} "
          f"({(d.score >= THRESH).mean():.1%}) | XLF-excluded + ADV>=10M next")

    report("FULL pt-in-time 2019-2026 (features NaN-heavy pre-2022)", simulate(fr.weekly_slate(build_cands(d, "2018-09-01", "2026-06-30"))))
    ...
    out = HERE / "archive" / "experiments" / "gate_decomposition_v6" / "rc12b_phase3_transfer.json"
    res = {}
