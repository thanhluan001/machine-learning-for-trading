"""RC-13 diagnostic — flag-decisive band economics (user concern 2026-09-09).

Question: when the pool is SP400-heavy, does is_sp400's level shift push
marginal SP400 events over the 0.33 bar that wouldn't clear on their own
merits — and are those trades positive EV or dilution?

Method: re-train V7's fold-OOS scoring (identical to 108), but score each
SP400 event twice with the SAME models — flag=1 (real) and flag=0
(counterfactual). Execute the validated policy (0.33, sim) in the real
world. Split executed SP400 trades:

  flag_decisive    min0 < 0.33 <= min1   (picked ONLY via the flag boost)
  flag_independent min0 >= 0.33          (would clear the bar anyway)

Report each band's economics on DEV folds 1-3 and holdout 2026H1.
"""
from __future__ import annotations

import importlib.util
import sys
from pathlib import Path

import numpy as np
import pandas as pd

HERE = Path(__file__).resolve().parent


def load(name, path):
    spec = importlib.util.spec_from_file_location(name, path)
    m = importlib.util.module_from_spec(spec)
    sys.modules[name] = m
    spec.loader.exec_module(m)
    return m


v108 = load("v108", HERE / "108_rc13_v7_full_bar.py")
v104 = v108.v104
fr = v108.fr
bt = v108.bt

DB = bt.DB
MATRIX_KEY = "/features/train_matrix_combined"
FEATURES = v108.FEATURES
GATES = v108.GATES
XLF = v108.XLF
THRESH = 0.33

try:
    sys.stdout.reconfigure(encoding="utf-8", errors="replace")
except Exception:
    pass


def build_cands(oos, w0, w1):
    m = ((oos.score >= THRESH) & (~oos.sector.isin(XLF)) & oos.pregap_return.notna()
         & oos.adv_pass
         & (pd.to_datetime(oos.report_date) >= pd.Timestamp(w0))
         & (pd.to_datetime(oos.report_date) <= pd.Timestamp(w1)))
    c = oos[m].copy()
    c["entry_date"] = pd.to_datetime(c.entry_date)
    rows = []
    for r in c.itertuples(index=False):
        pl = v104.get_prices(r.permaTicker)
        if pl is None:
            continue
        dates, closes = pl
        e = int(np.searchsorted(dates, np.datetime64(r.entry_date), side="left"))
        x = int(np.searchsorted(dates, np.datetime64(pd.Timestamp(r.exit_date)), side="left"))
        if e >= len(closes) or x >= len(closes) or x <= e:
            continue
        ret = fr.ret_with_stop(closes, e, x, stop=v104.STOP)
        if ret is None:
            continue
        rows.append({"permaTicker": r.permaTicker, "entry_date": r.entry_date,
                     "exit_date": pd.Timestamp(dates[x]), "score": r.score,
                     "score_noflag": r.score_noflag, "is_sp400": int(r.is_sp400),
                     "cost": float(r.cost), "entry_idx": e, "ret5": ret,
                     "ret_cost": ret - float(r.cost)})
    out = pd.DataFrame(rows)
    return out.sort_values(["entry_date", "score"], ascending=[True, False]).reset_index(drop=True)


def band_of(r):
    if r.is_sp400 == 0:
        return "sp600"
    return "flag_decisive" if r.score_noflag < THRESH <= r.score else "flag_independent"


def main() -> None:
    d = pd.read_hdf(DB, MATRIX_KEY)
    s6 = pd.read_hdf(v104.DB_SP600, "/features/train_matrix_sp600_pt",
                     columns=["permaTicker", "report_date", "adv20"])
    s6["report_date"] = pd.to_datetime(s6.report_date)
    d = d.merge(s6, on=["permaTicker", "report_date"], how="left")
    d["adv_pass"] = ~(d.adv20.notna() & (d.adv20 < 1e7))

    import xgboost as xgb
    frames = []
    for fi, tr, sv, ts in v108.folds_for(d):
        train = pd.concat([tr, sv], ignore_index=True)
        for g in GATES:
            y = train[g].astype(int).to_numpy()
            yt = ts[g].astype(int).to_numpy()
            clf = v108.fit(train[FEATURES], y, ts[FEATURES], yt)
            X1 = xgb.DMatrix(ts[FEATURES])  # flag as stored (1 for SP400, 0 SP600)
            ts[f"p_{g}"] = clf.predict_proba(ts[FEATURES])[:, 1]
            # counterfactual: same model, flag forced to 0
            Xcf = ts[FEATURES].copy()
            Xcf["is_sp400"] = 0.0
            ts[f"p0_{g}"] = clf.predict_proba(Xcf)[:, 1]
        ts["fold"] = fi
        ts["score"] = ts[[f"p_{g}" for g in GATES]].min(axis=1)
        ts["score_noflag"] = ts[[f"p0_{g}" for g in GATES]].min(axis=1)
        frames.append(ts)
        print(f"  fold {fi} scored (both flag variants)")
    oos = pd.concat(frames, ignore_index=True)

    v104._bulk_load()

    for label, windows in [("DEV folds 1-3", [(w0, w1) for _, w0, w1 in v104.FOLDS[:3]]),
                           ("HOLDOUT 2026H1", [("2026-01-01", "2026-06-30")])]:
        trades = []
        for w0, w1 in windows:
            trades.extend(v108.simulate(fr.weekly_slate(build_cands(oos, w0, w1))))
        t = pd.DataFrame(trades)
        t["band"] = t.apply(band_of, axis=1)
        print(f"\n[{label}] executed {len(t)} trades")
        for band in ("flag_independent", "flag_decisive", "sp600"):
            sub = t[t.band == band]
            if not len(sub):
                print(f"  {band:16s} n=0")
                continue
            r = sub.ret_cost.astype(float)
            lo, hi = v108.boot_ci(r.to_numpy())
            print(f"  {band:16s} n={len(sub):3d}  mean {r.mean():+.3%} "
                  f"CI[{lo:+.1%},{hi:+.1%}]  win {(r > 0).mean():.0%}")


if __name__ == "__main__":
    main()
