#!/usr/bin/env python3
"""
Pre-gap entry + time-delayed stop (skip gap day) backtest.

Scenario D: Pre-gap entry + 3% stop, but ONLY check from day 1 onward
  - Day 0 (gap/announcement day): no stop check — let the gap settle
  - Days 1..N: if Low[k] <= entry*0.97, exit at stop price
  - If no stop by exit day: exit at Close[exit_day]

This avoids the fatal flaw of the naive stop:
  - Gap-downs that recover aren't cut (day-0 dip is skipped)
  - Gradual declines after day 0 are still capped
"""
...
def compute_all_returns(trades_df, db_path, stop_loss=0.03, hold_days=5):
    """Compute returns for 4 scenarios:
    A. Current: Open[T+1] entry, Close[T+5] exit (no stop)
    B. Pre-gap no stop: Close[T-1]/Close[T] entry, Close[T+5] exit
    C. Pre-gap + naive stop: check from gap day onward
    D. Pre-gap + delayed stop: skip gap day, check from day 1 onward
    """
    results = []
    with pd.HDFStore(db_path, mode="r") as s:
        for _, trade in trades_df.iterrows():
            ...
            if is_bmo:
                entry_idx = t_idx - 1
                gap_day = t_idx          # day 0 (announcement day)
                exit_idx = t_idx + hold_days
            else:  # AMC
                entry_idx = t_idx
                gap_day = t_idx + 1      # day 0 (reaction day)
                exit_idx = t_idx + hold_days
            ...
            ret_a = float(c_t5 / o_t1 - 1.0) if (pd.notna(o_t1) and pd.notna(c_t5) and o_t1 > 0) else np.nan

            # --- Scenario B: Pre-gap no stop ---
            ret_b = float(c_t5 / entry_price - 1.0) if pd.notna(c_t5) else np.nan

            stop_price = entry_price * (1.0 - stop_loss)

            # --- Scenario C: Pre-gap + naive stop (check from gap_day) ---
            ret_c = np.nan
            stop_c_day = None
            for k in range(gap_day, exit_idx + 1):
                if k >= len(p_open):
                    break
                o_k = p_open[k]
                lo_k = p_low[k]
                if pd.notna(o_k) and o_k <= stop_price:
                    ...
                    stop_c_day = k - gap_day
                    break
                elif pd.notna(lo_k) and lo_k <= stop_price:
                    ret_c = float(stop_price / entry_price - 1.0)
                    stop_c_day = k - gap_day
                    break
            if pd.isna(ret_c) or ret_c is np.nan:
                ret_c = float(c_t5 / entry_price - 1.0) if pd.notna(c_t5) else np.nan

            # --- Scenario D: Pre-gap + delayed stop (skip gap_day, check from gap_day+1) ---
            ret_d = np.nan
            stop_d_day = None
            for k in range(gap_day + 1, exit_idx + 1):
                if k >= len(p_open):
                    break
                o_k = p_open[k]
                lo_k = p_low[k]
                if pd.notna(o_k) and o_k <= stop_price:
                    ret_d = float(o_k / entry_price - 1.0)
                    stop_d_day = k - gap_day
                    break
                elif pd.notna(lo_k) and lo_k <= stop_price:
                    ret_d = float(stop_price / entry_price - 1.0)
                    stop_d_day = k - gap_day
                    break
            if pd.isna(ret_d) or ret_d is np.nan:
                ret_d = float(c_t5 / entry_price - 1.0) if pd.notna(c_t5) else np.nan
            ...
            results.append({
                ...
                "ret_pregap_naivestop": ret_c,
                "ret_pregap_delayedstop": ret_d,
                "opening_gap": gap,
                "stop_naive_day": stop_c_day,
                "stop_delayed_day": stop_d_day,
                "stop_naive_hit": stop_c_day is not None,
                "stop_delayed_hit": stop_d_day is not None,
            })
...
def print_stats(df, col, label):
    ...
    print(f"  {label:<35} N={len(pnls):>3}  Win={wr:>5.1f}%  "
...
def main():
    print("=" * 95)
    print(f"PRE-GAP ENTRY + TIME-DELAYED STOP (skip gap day)")
    print(f"  theta={THETA}, {N_SLOTS} slots, {EXIT_SNAP}-day hold, {STOP_LOSS*100:.0f}% stop")
    print("=" * 95)
    ...
    print(f"\n[4] Computing returns for all 4 scenarios ...")
    results = compute_all_returns(all_df, DB, STOP_LOSS, EXIT_SNAP)
    print(f"  Computed returns for {len(results)} trades")

    # ===== ANALYSIS =====
    print(f"\n{'='*95}")
    print("RESULTS: 4 SCENARIOS COMPARED")
    print(f"{'='*95}")

    print(f"\n1. ALL TRADES ({len(results)})")
    print_stats(results, "ret_current", "A. Current (Open[T+1], no stop)")
    print_stats(results, "ret_pregap_nostop", "B. Pre-gap (no stop)")
    print_stats(results, "ret_pregap_naivestop", "C. Pre-gap + naive stop")
    print_stats(results, "ret_pregap_delayedstop", "D. Pre-gap + DELAYED stop")

    print(f"\n2. BMO TRADES ({int(results['is_bmo'].sum())})")
    bmo_df = results[results["is_bmo"]]
    print_stats(bmo_df, "ret_current", "A. Current (Open[T+1], no stop)")
    print_stats(bmo_df, "ret_pregap_nostop", "B. Pre-gap (no stop)")
    print_stats(bmo_df, "ret_pregap_naivestop", "C. Pre-gap + naive stop")
    print_stats(bmo_df, "ret_pregap_delayedstop", "D. Pre-gap + DELAYED stop")

    print(f"\n3. AMC TRADES ({int((~results['is_bmo']).sum())})")
    amc_df = results[~results["is_bmo"]]
    print_stats(amc_df, "ret_current", "A. Current (Open[T+1], no stop)")
    print_stats(amc_df, "ret_pregap_nostop", "B. Pre-gap (no stop)")
    print_stats(amc_df, "ret_pregap_naivestop", "C. Pre-gap + naive stop")
    print_stats(amc_df, "ret_pregap_delayedstop", "D. Pre-gap + DELAYED stop")

    print(f"\n4. PEAD TRADES ({int(results['pead_pass'].sum())})")
    pead_df = results[results["pead_pass"] == 1]
    print_stats(pead_df, "ret_current", "A. Current (Open[T+1], no stop)")
    print_stats(pead_df, "ret_pregap_nostop", "B. Pre-gap (no stop)")
    print_stats(pead_df, "ret_pregap_naivestop", "C. Pre-gap + naive stop")
    print_stats(pead_df, "ret_pregap_delayedstop", "D. Pre-gap + DELAYED stop")

    print(f"\n5. PEAD + BMO ({int(pead_df['is_bmo'].sum())})")
    pead_bmo = pead_df[pead_df["is_bmo"]]
    print_stats(pead_bmo, "ret_current", "A. Current (Open[T+1], no stop)")
    print_stats(pead_bmo, "ret_pregap_nostop", "B. Pre-gap (no stop)")
    print_stats(pead_bmo, "ret_pregap_naivestop", "C. Pre-gap + naive stop")
    print_stats(pead_bmo, "ret_pregap_delayedstop", "D. Pre-gap + DELAYED stop")

    print(f"\n6. PEAD + AMC ({int((~pead_df['is_bmo']).sum())})")
    pead_amc = pead_df[~pead_df["is_bmo"]]
    print_stats(pead_amc, "ret_current", "A. Current (Open[T+1], no stop)")
    print_stats(pead_amc, "ret_pregap_nostop", "B. Pre-gap (no stop)")
    print_stats(pead_amc, "ret_pregap_naivestop", "C. Pre-gap + naive stop")
    print_stats(pead_amc, "ret_pregap_delayedstop", "D. Pre-gap + DELAYED stop")

    # Stop comparison
    print(f"\n7. STOP COMPARISON (naive vs delayed)")
    naive_hit = results["stop_naive_hit"].sum()
    delayed_hit = results["stop_delayed_hit"].sum()
    print(f"  Naive stop hit:   {naive_hit}/{len(results)} ({naive_hit/len(results)*100:.1f}%)")
    print(f"  Delayed stop hit: {delayed_hit}/{len(results)} ({delayed_hit/len(results)*100:.1f}%)")

    # Trades where delayed stop saved us (naive triggered but delayed didn't)
    naive_only = results[results["stop_naive_hit"] & ~results["stop_delayed_hit"]]
    print(f"\n  Trades where NAIVE triggered but DELAYED didn't: {len(naive_only)}")
    if len(naive_only) > 0:
        print(f"    Naive stop return:   {naive_only['ret_pregap_naivestop'].mean()*100:+.2f}%")
        print(f"    Delayed stop return: {naive_only['ret_pregap_delayedstop'].mean()*100:+.2f}%")
        print(f"    No-stop return:      {naive_only['ret_pregap_nostop'].mean()*100:+.2f}%")
        print(f"    => Delayed saved {naive_only['ret_pregap_delayedstop'].mean() - naive_only['ret_pregap_naivestop'].mean():+.4f}/trade vs naive")

    # Worst-case comparison
    print(f"\n8. WORST-CASE COMPARISON (bottom 5 by pre-gap no-stop)")
    worst = results.nsmallest(5, "ret_pregap_nostop")[
        ["ticker", "is_bmo", "pead_pass", "ret_current", "ret_pregap_nostop",
         "ret_pregap_naivestop", "ret_pregap_delayedstop", "opening_gap",
         "stop_naive_hit", "stop_delayed_hit"]]
    print(worst.to_string(index=False, float_format=lambda x: f"{x*100:+.2f}%"))

    # Per-fold breakdown for the winning scenario
    print(f"\n9. PER-FOLD: Pre-gap + DELAYED stop")
    print(f"  {'Fold':<6} {'N':>4} {'Win%':>7} {'Avg':>10} {'Sharpe':>8}")
    for fi in range(1, 5):
        sub = results[results["fold"] == fi]
        if len(sub) == 0:
            continue
        pnls = sub["ret_pregap_delayedstop"].dropna()
        wr = (pnls > 0).mean() * 100
        avg = pnls.mean() * 100
        std = pnls.std()
        sharpe = (avg / 100) / (std + 1e-9) * np.sqrt(252 / 5) if std > 0 else 0  # rough annualization
        print(f"  {fi:<6} {len(pnls):>4} {wr:>6.1f}% {avg:>+9.2f}% {sharpe:>+7.2f}")
