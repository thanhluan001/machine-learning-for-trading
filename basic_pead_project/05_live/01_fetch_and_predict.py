...
# SKELETON NOTE: the V4 comparison model is optional here — the live book
# runs on V6 gates. If the V4 model files are absent, the V4 comparison
# leg is skipped gracefully (p_v4_pead reported as None).
V4_ENABLED = FROZEN_CLASSIFIER.exists() and FROZEN_META.exists()
...
def main(weeks: int = 2, dry_run: bool = False, limit: int | None = None,
    ...
    if not FROZEN_CLASSIFIER.exists():
        print(f"\n  NOTE: V4 comparison model not found ({FROZEN_CLASSIFIER}) —")
        print("  running V6-gates only (the live path).")
    ...
    with pd.HDFStore(DB_FILE, mode="r") as lazy_store:
        for idx, (_, row) in enumerate(cal_sp400.iterrows()):
            ...
            proba = float(clf.predict_proba(X)[0, 1]) if clf is not None \
                else v6_scores["p_v6_min"]
            ...
            results.append({
                ...
                **{k: round(v, 4) for k, v in v6_scores.items()},
                "p_v4_pead": round(proba, 4) if clf is not None else None,
    ...
    if clf is None:
        accepted_v4, picks_v4 = [], []
        print("  V4 comparison skipped (model absent in this skeleton).")
    else:
    ...
    plan_v4 = _make_plan("phase_g_v4_timing_correct", picks_v4, THETA, len(accepted_v4), {
        "status": "comparison_only_not_executed", "accepted_after_theta": sum((r["p_v4_pead"] or 0) >= THETA for r in results) if clf is not None else 0,
        "accepted_after_xlf": len([r for r in results if (r["p_v4_pead"] or 0) >= THETA and r["sector"] not in EXCLUDE_SECTORS]) if clf is not None else 0,
    })
    ...
    # --- Step 8: combined-universe shadow — NOT PART OF THIS SKELETON
    # (research companion excluded; the live pipeline is unaffected). ---
